...
drop index if exists ix_standard_drug_legacy_outcome_count_4;
create index ix_standard_drug_legacy_outcome_count_4  on faers.standard_drug_legacy_outcome_count  using btree  (drug_outcome_pair_count);

analyze verbose standard_drug_legacy_outcome_count;

-- get count_d1 -- 1 second  
drop table if exists standard_drug_legacy_outcome_count_d1;
create table standard_drug_legacy_outcome_count_d1 as
with cte as (
select sum(drug_outcome_pair_count) as count_d1 from standard_drug_legacy_outcome_count 
)  
select concept_id, outcome_concept_id, count_d1
from standard_drug_legacy_outcome_count a,  cte -- we need the same total for all rows so do cross join!
...
set search_path = faers;
drop table if exists standard_drug_legacy_outcome_count_a_count_b;
create table standard_drug_legacy_outcome_count_a_count_b as
...
from standard_drug_legacy_outcome_count a;

-- get count_c -- 35.45 minutes
set search_path = faers;
drop table if exists standard_drug_legacy_outcome_count_c;
create table standard_drug_legacy_outcome_count_c as
select concept_id, outcome_concept_id, 
(
	select sum(drug_outcome_pair_count) 
...
set search_path = faers;
drop table if exists standard_drug_legacy_outcome_count_d2;
create table standard_drug_legacy_outcome_count_d2 as
select concept_id, outcome_concept_id, 
(
	select sum(drug_outcome_pair_count)
...
create table standard_drug_legacy_outcome_contingency_table as		-- 6 seconds
select ab.concept_id, ab.outcome_concept_id, count_a, count_b, count_c, (count_d1 - count_d2) as count_d
from standard_drug_legacy_outcome_count_a_count_b ab
inner join standard_drug_legacy_outcome_count_c c
on ab.concept_id = c.concept_id and ab.outcome_concept_id = c.outcome_concept_id
inner join standard_drug_legacy_outcome_count_d1 d1
on ab.concept_id = d1.concept_id and ab.outcome_concept_id = d1.outcome_concept_id
inner join standard_drug_legacy_outcome_count_d2 d2
