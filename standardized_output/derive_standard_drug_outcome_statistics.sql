...
--
-- It then processes the 2x2 contingency table to create a statistics table called standard_drug_outcome_stats with the following statistics for each drug/outcome pair:
--
-- 1) case_count
...
create index ix_standard_drug_outcome_count on standard_drug_outcome_count(concept_id, outcome_concept_id);
drop index if exists ix_standard_drug_outcome_count_2;
create index ix_standard_drug_outcome_count_2  on faers.standard_drug_outcome_count  using btree  (concept_id);
...
analyze verbose standard_drug_outcome;
...
from 
 (
	select concept_id, outcome_concept_id, drug_outcome_pair_count as count_a, -- drug P and outcome R
		...
		(
			select sum(drug_outcome_pair_count)
			from standard_drug_outcome_count d1
	...
	from standard_drug_outcome_count a -- count of all cases where drug P and outcome R
) contingencytable;
