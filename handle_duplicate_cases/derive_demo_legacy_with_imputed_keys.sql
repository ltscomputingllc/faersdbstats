...
-- create table of default demo legacy key values for each case where all the key fields are populated on at least one report for that case
drop table if exists demo_legacy_with_imputed_keys;
create table demo_legacy_with_imputed_keys as select * from demo_legacy; 

drop table if exists default_demo_legacy_keys; 
